...
// Vendored from the RandomNerdTutorials ESP32 CYD example - see the
// header for source/attribution. One line fixed from the original: the
// "dummy X measure" call still used the bare hardware `SPI` object
// instead of `touchscreenSPI` (a leftover from the hardware-SPI library
// this was forked from) - left as-is, that line would clock a stray
// transaction onto the TFT's own SPI bus on every touch.
...
void XPT2046_TouchscreenSOFTSPI::update(SoftSPI *touchscreenSPI)
{
	...
	if (z >= Z_THRESHOLD) {
		...
		touchscreenSPI->transfer16(0x91 /* X */); // dummy X measure, 1st is always noisy (fixed: was bare SPI.)
